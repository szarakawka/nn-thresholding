...
for i, pi in etc.pi_sequence_timer(test_string, info_frequency=1):
    sys.stdout.write(i)
    sys.stdout.flush()
    if pi:
        sys.stdout.write('\n')
...
for i, pi in etc.pi_file_timer(open('etc_examples.py', 'r'), info_frequency=1):
    if pi:
        print("progress at %g"%pi.progress)
        print("%g seconds elapsed"%pi.elapsed_time)
        print("%g seconds remaining"%pi.time_remaining())
